...
## 十二、按核数动态定 task 粒度（本版真正省时间的一刀）

**问题**：v1~v7 的 task = 一个 batch ⇒ `b=1` 时全场只有 **1 个** AIC+AIV 对在跑，其余
23 个核空转。这时 CrossCoreWait 的大头不是"消息慢"，而是"没活干"——参考时间 3.6~91µs
的判题形状，几乎都是小 batche。
...
| 项 | 旧 | 新 |
| --- | --- | --- |
| task 粒度 | (batch) | **(batch, 行块)**，task 数 = `b·⌈m/baseM⌉` |
| b=1、m=4096 时的 task 数 | **1** | **32**（24 个 cube core 基本喂满） |
| blocks | min(可用核, cube 核, 行块数) | 同上，再按 staging 预算与 task 数收缩 |
| 跨 task 归并 | 无（每 batch 一个属主） | **32B 对齐原子加**到 `[batch][8]` 槽（槽内 `[和,0×7]`），host 前置 0 |
| 写回 y | AIV 直接 `DataCopyPad` 到 y | 主机 `aclrtMemcpy` **纯搬运**（每 batch 第 0 个 float → y） |

**为什么"行块"这一刀是安全的**：每个 task 独占**整行**（N 方向不切），所以行内
（max over N、sum over rows）都在 task 内部完成，跨 task 只剩"行块和相加"这一件事 ——
一件不受顺序影响语义、只影响末位的事。

**为什么用 32B 原子槽而不是让 host 求和**：`SetAtomicAdd` 要求 32B 对齐的块
（`SetAtomicAdd.md:41,45`），所以槽里放 `[和, 0,0,0,0,0,0,0]`；而 host 侧只做
`aclrtMemcpy` —— **不做任何算术**。队友那版是把跨 chunk 的 max 和行和放到 Host CPU 上算的
（快，但撞赛题规则"核心计算必须在 NPU 上"）。本版保留在 NPU 上，代价只是末位不可复现
（相对误差 ~1e-7，判据是 `atol+rtol·|golden| ≈ 1`）。

**代价与已知风险**：

- 原子加顺序不定 ⇒ 结果**不是逐位可复现**（误差在末位，远小于判据）。
- 依赖 host 传的 `workerCount = 2*blocks` 等于**实际** AIV 数；否则有 task 没人领 →
  结果偏小。基线也是这个假设（真机 15/15 成立），已在 VERIFY 0) 里点名。
