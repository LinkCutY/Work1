...
# ----------------------------------------------------- 5. MmaMaxKernel (+y)
expect(2550, '    GM_ADDR x1,')
expect(2551, '    GM_ADDR x2,')
expect(2552, '    GM_ADDR partial,')
expect(2553, '    GM_ADDR cWorkspace,')
expect(2554, '    Shape shape,')
add(2552, ['    GM_ADDR y,'])
repl[2496] = ' * path produces, so the phase-3 merge is shared.'

expect(3479, '            Fence<HardEvent::MTE3_S>(pipe);')
expect(3480, '        }')
expect(3481, '    }')
expect(3482, '}')
expect(3485, 'template <typename InputT>')
add(3482, [
    '',
    '    /*',
...
expect(3487, '    GM_ADDR x1,')
expect(3488, '    GM_ADDR x2,')
expect(3489, '    GM_ADDR partial,')
expect(3490, '    GM_ADDR cWorkspace,')
expect(3491, '    Shape shape,')
expect(3492, '    uint32_t blocks,')
add(3489, ['    GM_ADDR y,'])

# call sites: 4 x MmaMaxKernel (compact one-liners)
for i in (3507, 3517, 3527, 3534):
...
# ------------------------------------------------------- 7. stale comments
expect(3887, '    /*')
expect(3888, '     * Phase 2 runs on the host: a plain-AIV')
expect(3894, '     */')
for i in range(3887, 3895):
    drop.add(i)
add(3887, [
    '    /*',
...
expect(4031, '     * layout and host merge as the cube')
repl[4031] = '     * layout and the on-device merge of'
expect(4032, '     * path, just a different nChunks.')
repl[4032] = '     * the cube path, just different nChunks.'

# ------------------------------------------------- 8. drop the host merge
expect(4823, '    CheckAcl(')
expect(4827, '    );')
expect(4830, '    /*')
expect_has(4831, 'Phase 2 on host')
expect(4841, '        hostPartial;')
expect(4959, '            ACL_MEMCPY_HOST_TO_DEVICE)')
expect(4960, '    );')
expect(4963, '}')
for i in range(4830, 4961):
    drop.add(i)
add(4830, [
    '    /*',
